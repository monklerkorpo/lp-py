...
const state = {
  ...
  const elements = {
    ...
    method: document.getElementById('method'), // Добавлен элемент для выбора метода
  };
  
  // Обновление полей целевой функции
  function updateObjectiveInputs() {
    console.log('Обновление полей целевой функции...');
    ...
    console.log('Поля целевой функции обновлены:', elements.objectiveInputs.innerHTML);
    updateConstraints();
  }
  
  // Обновление ограничений
  function updateConstraints() {
    console.log('Обновление ограничений...');
  ...
  function addConstraint() {
    console.log('Добавление нового ограничения...');
    ...
    console.log('Ограничение добавлено:', div.innerHTML);
  }
  
  // Загрузка шаблонов
  function loadTemplate() {
    console.log('Загрузка шаблона:', elements.template.value);
  ...
  function validateInputs() {
    ...
    const method = elements.method.value; // Получаем выбранный метод
    ...
    constraintElems.forEach((elem, idx) => {
      ...
      // Проверка метода симплекса с ограничениями ">=" или "="
      if (method === 'simplex' && (type === '>=' || type === '=')) {
        errors.push(`Для метода "Симплекс" недопустимы ограничения типа "${type}" в ограничении "${resourceName}"`);
      }
    });
  ...
  async function solve() {
    console.log('Запуск решения задачи...');
    clearErrors();
    ...
    try {
      ...
      console.log('Результат получен:', state.lastResult);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    updateObjectiveInputs(); // Инициализация полей
